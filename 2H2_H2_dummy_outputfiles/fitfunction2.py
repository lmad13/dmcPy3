...
if dimension == '2': 
    ...
    def model(variables, c1, a2, b2, h2, a3, b3, h3,  a4, b4, h4, a5, b5, h5, k):
        x, y = variables
        gy = a2*np.sin(b2*(y-h2)) + a3*np.sin(b3*(y-h3)) + a4*np.sin(b4*(y-h4)) + a5*np.sin(b5*(y-h5))
        fx = np.exp(-1*c1*x)
        return fx * gy + k
    
    initial_fit = [3.2682585903872523,
                   -0.01179499771,3.0619396556882577,-2.2704746836396392,
                   0.020331052386811668,0.470863139380064,-13.53837436260545,
                    -0.004613970569229372,3.0619396553702964,-0.28308974887096405,
                   0.0203,0.4708,-13.5383,
                   -0.004418821301130721]
    
    fit = curve_fit(model, (combinex, combiney), combinez, initial_fit)
    
    ans, cov = fit
    
    fit_c1, fit_a2, fit_b2, fit_h2, fit_a3, fit_b3, fit_h3,  fit_a4, fit_b4, fit_h4, fit_a5, fit_b5, fit_h5, fit_k = ans

    modeledz = model((combinex, combiney), fit_c1, fit_a2, fit_b2, fit_h2, fit_a3, fit_b3, fit_h3,  fit_a4, fit_b4, fit_h4, fit_a5, fit_b5, fit_h5, fit_k)
